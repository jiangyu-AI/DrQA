...
def process_dataset(data, tokenizer, workers=None):
    ...
    try:
        ...
        print('data[question]: ' + str(data['questions']))
    ...
    for idx in range(len(data['qids'])):
        ...
        if len(data['answers']) > 0:
            for ans in data['answers'][idx]:
                try: # added by yu to extend to Chinese sentence level ans
                    found = find_answer(offsets,
                                    ans['answer_start'],
                                    ans['answer_start'] + len(ans['text']))
                    if found:
                        ans_tokens.append(found)
                except: # for Chinse sentence level ans
                    if ans['id']:
                        ans_tokens.append(ans['id'])
        yield {
            'id': data['qids'][idx],
            'question': question,
            'document': document,
